...
func RetrieveMaterialsFromArtifacts(obj *objects.TaskRunObject) []slsa.ProvenanceMaterial {
	// Retrieve structured provenance for inputs.
	mats := []slsa.ProvenanceMaterial{}
	if obj.Status.Artifacts != nil {
		for _, oa := range obj.Status.Artifacts.Outputs {
			uri := oa.Value.ObjectVal["uri"]
			digest := oa.Value.ObjectVal["digest"]
			mats = append(mats, slsa.ProvenanceMaterial{
				URI:    uri,
				Digest: map[string]string{"sha256": digest},
			})
		}
		for _, ia := range obj.Status.Artifacts.Inputs {
			uri := ia.Value.ObjectVal["uri"]
			digest := ia.Value.ObjectVal["digest"]
			mats = append(mats, slsa.ProvenanceMaterial{
				URI:    uri,
				Digest: map[string]string{"sha256": digest},
			})
